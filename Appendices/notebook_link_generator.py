...
def generate_notebook_link(notebook_path, canvas_id, course_id, target_path=None, user_id=None):
    """
    ...
    - canvas_id (str): The Canvas course ID such as "67502".
    - course_id (str): The course ID such as "cs5483_25b".
    - target_path (str): The target path. Default: course_id
    - user_id (str): User ID. Default: The login user
    ...
    if target_path is None:
        target_path = course_id
    # Notebook URL generation
    # 1. Construct the URL for git-pull service to clone or pull the notebook repository
    gitpull_url = (
        f"{course_server}/{course_id}/{'hub/user-redirect' if user_id is None else 'user/'+user_id}/git-pull?"
        + f"repo={quote(notebook_repo)}"
        + f"&urlpath={quote(f'lab/tree/{target_path}/{notebook_path}')}"
        + (f"&targetPath={quote(f'{target_path}')}")
    )
...
def setup_notebook_link_widget(notebook_path, canvas_id, course_id, target_path=None, user_id=None):
    """
    ...
    html_output = widgets.HTML(
        value=generate_notebook_link(text_input.value, canvas_id=canvas_id, course_id=course_id, target_path=target_path, user_id=user_id)  # Generate initial link
    )
